...
/*   Updated: 2024/04/24 20:55:59 by craimond         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_window(t_mlx_data *win_data, t_scene *scene)
{
	...
	win_data->aspect_ratio
		= (double)win_data->win_width / (double)win_data->win_height;
	win_data->viewport_x
	...
	while (shapes)
	{
		...
		if (material->texture)
		{
			texture_img = mlx_xpm_file_to_image(mlx_data->mlx,
					material->texture->path, &material->texture->width,
					&material->texture->height);
			if (!texture_img)
				ft_quit(41, "mlx: failed to load texture");
			material->texture->addr = mlx_get_data_addr(texture_img,
					&bits_per_pixel, &material->texture->line_length,
...
static int	key_hook(const int keycode, t_hook_data *hook_data)
{
	...
	else if (keycode == KEY_SPACE && win_data->n_images > 1)
	{
		...
		mlx_put_image_to_window(win_data->mlx,
			win_data->win, win_data->images[win_data->current_img], 0, 0);
	}
	return (0);
}
