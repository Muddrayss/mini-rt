...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/24 20:56:39 by craimond          #+#    #+#             */
/*   Updated: 2024/04/24 21:04:00 by craimond         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	intersect_ray_sphere(const t_ray ray, const t_shape *shape)
{
	...
	const double		c = vec_dot(oc, oc) - shape->sphere.radius  shape->sphere.radius;
	...
	else
	{
		...
		t12[0] = (-b - sqrt_discriminant) / 2;
		t12[2] = (-b + sqrt_discriminant) / 2;
		if (t12[0] < 0 && t12[2] < 0)
			return (-1);
		if (t12[0] > 0 && t12[2] > 0)
			return (fmin(t12[0], t12[2]));
		else if (t12[0] > 0)
			return (t12[0]);
		else if (t12[2] > 0)
			return (t12[2]);
