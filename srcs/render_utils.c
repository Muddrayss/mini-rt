...
/*   Updated: 2024/04/23 16:40:48 by egualand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	pthread_attr_t *thread_attr)
{
	...
	while (i < N_THREADS)
	{
		...
		threads_data[i]->scene = s_and_m.scene;
		threads_data[i]->win_data = s_and_m.mlx_data;
		threads_data[i]->light_ratios = light_ratios;
		threads_data[i]->start_y = i * lines_per_thread;
		if (i == N_THREADS - 1)
			threads_data[i]->end_y = s_and_m.mlx_data->win_height;
		else
